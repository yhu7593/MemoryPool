...
void testEdgeCases()
{
    ...
    // 测试最大大小边界
    void *ptr3 = MemoryPool::allocate(MAX_SIZE);
    assert(ptr3 != nullptr);
    MemoryPool::deallocate(ptr3, MAX_SIZE);

    // 测试超过最大大小
    void *ptr4 = MemoryPool::allocate(MAX_SIZE + 1);
    assert(ptr4 != nullptr);
    MemoryPool::deallocate(ptr4, MAX_SIZE + 1);
